...
class _function {
    ...
    public function Paginate( $options = [], $opsPageNum = [] ){

        if( empty($_GET['pager']) ) $_GET['pager'] = 1; 
        if( empty($options['geturl']) ) $options['geturl'] = $_GET;

        $pager = array(
            'length'=>$options['pager']
        );

        $opsPageNum['shownum'] = isset($opsPageNum['shownum']) ? $opsPageNum['shownum'] : true;

        $pager['prev'] = $pager['length']-1;
        $pager['prev_url'] = $this->getURL($options['geturl'], "pager", ($pager['length']-1));
        $pager['prev_btn'] = $pager['prev']<1
            ? '<a class="phs btn disabled" href="#"><i class="icon-chevron-left"></i> กลับ</a>'
            : '<a class="phs btn" href="'.$pager['prev_url'].'"><i class="icon-chevron-left"></i> กลับ</a>';

        $pager['next'] = $pager['length']+1;
        $pager['next_url'] = $this->getURL($options['geturl'], "pager", ($pager['length']+1));
        $pager['next_btn'] = '<a class="phs btn" href="'.$pager['next_url'].'">ถัดไป <i class="icon-chevron-right"></i></a>';

        $limit = $options['limit'];
        $start_limit = ($limit*$pager['length'])-$limit+1;
        $end_limit = $limit*$pager['length'];

        if($end_limit>=$options['total']){
            $end_limit = $options['total'];
            $pager['next_btn'] = '<a class="phs btn disabled" href="#">ถัดไป <i class="icon-chevron-right"></i></a>';
        }

        if( !empty($options['is_disabled']) ){
            $pager['prev_btn'] = '<a class="phs btn disabled" href="#"><i class="icon-chevron-left"></i> กลับ</a>';
            $pager['next_btn'] = '<a class="phs btn disabled" href="#">ถัดไป <i class="icon-chevron-right"></i></a>';
        }
        ...
        $btnWarpStart = '';
        $btnWarpEnd = '';
        if( !empty($opsPageNum['shownum']) ){
            $totalPage = ceil( ( $options['total'] / $options['limit'] ) );
            $start = 1;
            $end = $totalPage;

            if( !empty($opsPageNum['short']) ){
                $setStartPage = $options['pager']-2;
                $setEndPage = $options['pager']+2;
                if( $options['pager'] != 1 && $setStartPage > 1 ) $start = $setStartPage;
                if( $setEndPage < $totalPage ) $end = $setEndPage;
            }

            if( !empty($opsPageNum['warp']) ){
                $btnWarpStart = '<a class="phs btn" href="'.$this->getURL($options['geturl'], "pager", 1).'">'.
                                    '<i class="icon-angle-double-left"></i>'.
                                ' หน้าแรก</a>';
                $btnWarpEnd = '<a class="phs btn" href="'.$this->getURL($options['geturl'], "pager", $totalPage).'">'.
                                ' หน้าสุดท้าย'.
                                '<i class="icon-angle-double-right"></i></a>';
            }

            for($i=$start; $i<=$end; $i++){
                $pageURL = $this->getURL($options['geturl'], "pager", ($i));
                $disabled = '';
                $active = '';
                if( $i == $options['pager'] ) {
                    $pageURL = '#';
                ...
                $btnNumber .= '<a class="phs btn '.$active.' '.$disabled.'" href="'.$pageURL.'">'.$i.'</a>';
            }
        }

        return '<li>'.
                '<div class="lfloat">'.
                    // '<span class="mhs fcg">แสดงผลหน้าละ '.$options['limit'].' แถว</span><br/>'.
                    '<span class="mhs fcg">แสดง '.$start_limit.' ถึง '.$end_limit.' จาก '.$options['total'].' รายการ</span>'.
                '</div>'.
                '<div class="rfloat r group-btn">'.
                    $btnWarpStart.$pager['prev_btn'].$btnNumber.$pager['next_btn'].$btnWarpEnd.
                '</div>'.
              '</li>';
    }
    ...
    public function imageWideBox($url, $size=640, $theSize=array(640, 360)){
        $width = $size;
        // $height = round( ($theSize[1]*$size) /$theSize[0], 2 );
        return '<div class="avatar-cover" style="width:'.$width .'px;height:auto;"><img src="'.$url.'" /></div>';
    }
    ...
    public function form_control($arr){
         $str = '';
        foreach ($arr as $value) {
            ...
            if($type=='select'){
                ...
                $input = '<select id="'.$id.'" name="'.$name.'" class="form-control'.$cls.'"'.$attr.'>'.$option.'</select>';
            }
            else{

                $val = isset($value['value']) ? ' value="'.$value['value'].'"':'';
                $input = '<input id="'.$id.'" autocomplete="off" placeholder="'.$value['label'].'" class="form-control'.$cls.'" type="text" name="'.$name.'"'.$attr.''.$val.'>';
            }
