...
class Media extends Controller  {
    ...
    public function del($id=''){
        ...
        $item = $this->model->load('media')->get( $id );
        if( empty($item) ) $this->error();


        if( !empty($_POST) ){

            $this->model->load('media')->del($id);
    ...
    public function set_sequence() {
   	...
   	public function set_data_slideshow() 	{
   		...
   		$item = $this->model->load('albums')->get_media( $id );
   		if( empty($item) ) $this->error();
   		...
   		$this->model->load('albums')->up_media( $id, array('media_caption'=> json_encode($data) ) );
