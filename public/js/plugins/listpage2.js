...
(function( $, window, document, undefined ) {
	...
	var List = {
		...
		setMore: function () {
			...
			self.$elem.find('#total-list').empty();
			if( total==0 ){
				self.$elem.find('#more-link').addClass('hidden_elem');
				self.$elem.find('#total-list').addClass('hidden_elem');
				return false;
			} 
			...
			if( self.$elem.find('#total-list').hasClass('hidden_elem') ){
				self.$elem.find('#total-list').removeClass('hidden_elem');
			}
			...
			self.$elem.find('#total-list').empty();
			self.$elem.find('#total-list').append( $('<span>', {class: 'mhs'}).append( PHP.number_format(total) , $('<span>', {class: 'mhs', text:'รายการ'}) ) );
		},
